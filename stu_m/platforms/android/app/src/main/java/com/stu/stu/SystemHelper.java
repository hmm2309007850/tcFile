...
public class SystemHelper extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("MoveToTop")) {
            String message = args.getString(0);
            Log.e("SystemHelper", message);
            //this.myMethod(message, callbackContext);
            //new MainActivity().setTopApp();
            //MyApplication.mActivity.showPopup(message);
            //test();
            //showPopup(message);
            callbackContext.success("MoveToTop");
            return true;
        } else if (action.equals("")) {
            Constant.INSTANCE.setSESSION(args.getString(0));
            callbackContext.success("拿到了Session");
        }
        callbackContext.error("没有对应action");
        return false;
    }
        ...
        if (!isRunningForeground(context)) {
            ...
            for (ActivityManager.RunningTaskInfo taskInfo : taskInfoList) {
                ...
                if (taskInfo.topActivity.getPackageName().equals(context.getPackageName())) {
                    ...
                    Log.e("SystemHelper", "已经把我自己切换到前台啦");
